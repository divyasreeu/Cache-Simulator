...
void accessData (
    ...
    /* ... */
    unsigned char index = (addr >> b) & (S-1);
    ...
    for (line = 0; line < E; line++) {
        cache_line_t* curr_line = cache_set + line;
        if ( (curr_line->valid == true) && (curr_line->tag == tag )) {
    ...
    line = 1;
    do {
        if (curr_line->recent_use > (cache_set + line)->recent_use ) {
            curr_line = cache_set + line;
        }
        line++;
    } while (line < E);
    ...
    /* ... */
    if (curr_line->valid == true) {
        (*eviction_count)++;
    }

    /* ... */
    curr_line->tag = tag;
    curr_line->valid = true;
    curr_line->recent_use = program_counter;
}
